...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        type=int,
        help="number of MLUs per node (default: None, use all available MLUs)",
    )
    parser.add_argument(
        "--save",
        metavar="DIR",
    ...
    parser.add_argument(
        ...
        default=64,
        type=int,
        help="batch size for single MLU (default: 64)",
    )
    parser.add_argument(
        "--lr",
        "--learning-rate",
        ...
        default=0.025,
        type=float,
        help="learning rate for single MLU (default: 0.025)",
    )
    parser.add_argument(
    ...
    if args.nmlus is None:
        args.nmlus = dist.helper.get_device_count_by_fork("cambricon")

    if args.world_size * args.nmlus > 1:
        dist_worker = dist.launcher(
            master_ip=args.dist_addr,
            port=args.dist_port,
            world_size=args.world_size * args.nmlus,
            rank_start=args.rank * args.nmlus,
            n_gpus=args.nmlus,
            backend="cncl"
        )(worker)
        dist_worker(args)
